...
2.  The data is valuable to researchers interested in the interplay of physiological and subjective stress. It enables exploratory data-analysis regarding individual differences in stress reactivity and mediators of the stress response.

3.  The unprocessed behavioral data may be used to investigate stress and behavior interactions in economic choice.

4.  The pre-processed data might be used for evidence aggregation in the field of choice consistency/rationality in the future.
...
```{r fig4-cortisol, echo=FALSE, message=FALSE, warning=FALSE, results=FALSE, fig.cap="Fig. 4: Cortisol and Amylase levels in stress and control groups."}

# run code from code directory instead of copy/pasting
# relative path problems...solution: change global Knitr options (knit from project directory - but this creates problems for collaborators who have diff. global IDE settings)
# solution: load all lib, data, global settings up front (top setup code chunks)
